namespace Assets.Modules.PlayerModule
{
    ...
    public class PlayerLocomotion : MonoBehaviour
    {
        ...
        {
            // Используем Transform.localPosition напрямую для скорости
            Transform camTransform = _camera.transform;
            float currentVelocity = 0f;

            // Цикл работает, пока не достигнет цели или не будет отменен
            while (Mathf.Abs(camTransform.localPosition.y - targetY) > 0.001f)
            {
                ...
                // await UniTask.Yield делает паузу до следующего кадра, не блокируя поток
